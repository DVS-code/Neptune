...
# Distance runs, measured from a standstill. These are timed by integrating speed rather
# than by watching for a speed crossing, so they are kept separate from the speed presets.
METRES_PER_MILE = 1609.344
DISTANCE_PRESETS = [
    ('60 ft', METRES_PER_MILE / 88.0),      # 60 feet, the drag-strip launch marker
...
class DragyModule(FeatureModule):
    ...
    def _tick_distance(self, speed: float, now: float) -> None:
        ...
        if self._state == ARMED:
            # Arm from a standstill, then start on the first movement. Idle creep means
            # "stopped" has to be a small band, not exactly zero.
            self._below_start = self._below_start or speed <= STANDSTILL_MS
            if not self._below_start:
                if speed <= STANDSTILL_MS:
                    self._below_start = True
                return
            if speed > STANDSTILL_MS:
                # Extrapolate back to a true zero-speed start. The run is only noticed
                # once the car passes the idle-creep threshold, by which point it is
                # already moving; timing from that instant starts the clock late, and
                # the later it starts the gentler the launch — a bias of tens of
                # milliseconds on a slow car and none at all on a fast one.
                step = now - stamp
                ...
                # Distance already covered since that extrapolated start.
        ...
        if step <= 0.0:
            return
        # Trapezoid: the average of the two samples over the interval. Using either end
        # alone biases every run, and the error compounds over a whole quarter mile.
        ...
        # Interpolate inside this interval so the result does not quantise to the tick.
    ...
    def load_state(self, data: dict) -> None:
        ...
        # Put the selectors back where the preset says, then show the rows that belong to
        # the restored mode — without this a loaded preset applied the right run but left
        # the previous mode's controls on screen.
        # `unit` is in this list because a preset saved in km/h used to restore the run
        # correctly while the selector still read mph, and the From/To sliders showed
        # values that belonged to a unit the interface was not displaying.

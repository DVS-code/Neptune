...
DECODERS = {
    71: texture2ddecoder.decode_bc1,
    72: texture2ddecoder.decode_bc1,
    77: texture2ddecoder.decode_bc3,
    78: texture2ddecoder.decode_bc3,
    98: texture2ddecoder.decode_bc7,
    99: texture2ddecoder.decode_bc7,
}
...
def decode(data: bytes) -> tuple[int, int, bytes]:
    ...
    pair = (
        TRANSCODING_TO_DXGI.get(transcoding) if transcoding > 1 else ENCODING_TO_DXGI.get(encoding)
    )
    if not pair:

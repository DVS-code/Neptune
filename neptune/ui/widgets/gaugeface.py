...
class GaugeFace(QWidget):
    ...
    def _paint_handle(self, painter: QPainter) -> None:
        ...
        radius = min(rect.width(), rect.height()) * (0.5 if self.mode == "Dial" else 0.12)
    ...
    def _paint_dial(self, painter: QPainter) -> None:
        ...
        track = QRectF(
            centre.x() - track_radius, centre.y() - track_radius, track_radius * 2, track_radius * 2
        )
        ...
        if filled > 0.5:
            if self.glow:
                for spread, alpha in ((2.6, 26), (1.7, 46)):
                    halo = QColor(accent)
                    halo.setAlpha(alpha)
                    painter.setPen(QPen(halo, thickness * spread, Qt.SolidLine, Qt.RoundCap))
        ...
        painter.drawText(
            QRectF(centre.x() - radius, centre.y() - radius * 0.30, radius * 2, radius * 0.46),
            Qt.AlignCenter,
            f"{self._value:.1f}",
        )
        ...
        painter.drawText(
            QRectF(centre.x() - radius, centre.y() + radius * 0.16, radius * 2, radius * 0.22),
            Qt.AlignCenter,
            self._unit.upper(),
        )
    ...
    def _paint_needle(
        self, painter: QPainter, centre: QPointF, radius: float, accent: QColor
    ) -> None:
        ...
        path = QPainterPath(
            QPointF(centre.x() + math.cos(angle) * outer, centre.y() - math.sin(angle) * outer)
        )
        path.lineTo(
            QPointF(
                centre.x() + math.cos(angle + span) * inner,
        ...
        if self.show_peak and self._peak > self._minimum:
            peak_angle = math.radians(START_ANGLE - self._fraction(self._peak) * SWEEP_ANGLE)
            painter.setPen(QPen(QColor(accent).lighter(170), max(1.6, radius * 0.022)))
            painter.drawLine(
                QPointF(
                    centre.x() + math.cos(peak_angle) * radius * 0.60,
                    centre.y() - math.sin(peak_angle) * radius * 0.60,
                ),
                QPointF(
                    centre.x() + math.cos(peak_angle) * radius * 0.715,
    ...
    def _paint_extras(
        ...
        painter.drawText(
            QRectF(centre.x() - radius, centre.y() - radius * 0.56, radius * 2, radius * 0.22),
            Qt.AlignCenter,
            self._label,
        )
        ...
        painter.drawText(
            QRectF(centre.x() - radius, centre.y() + radius * 0.40, radius * 2, radius * 0.26),
            Qt.AlignCenter,
            str(int(self._gear)),
        )
    ...
    def _paint_digital(self, painter: QPainter) -> None:
        ...
        painter.drawText(
            rect.adjusted(0, rect.height() * 0.10, 0, 0), Qt.AlignHCenter | Qt.AlignTop, self._label
        )
        ...
        if self.glow:
            ...
            painter.drawText(
                rect.adjusted(1.5, 1.5, 1.5, 1.5), Qt.AlignCenter, f"{self._value:.1f}"
            )
        ...
        painter.setFont(footer_font)
        painter.drawText(
            rect.adjusted(0, 0, 0, -rect.height() * 0.08), Qt.AlignHCenter | Qt.AlignBottom, footer
        )
    ...
    def _paint_bar(self, painter: QPainter) -> None:
        ...
        painter.setFont(label_font)
        painter.drawText(
            rect.adjusted(rect.width() * 0.06, 0, 0, -rect.height() * 0.10),
        ...
        painter.setFont(value_font)
        painter.drawText(
            rect.adjusted(0, 0, -rect.width() * 0.06, -rect.height() * 0.10),

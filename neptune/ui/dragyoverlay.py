...
class DragyOverlay(QWidget):
    ...
    def update_run(self, run: str, state: str, result, started, speed, unit, history=None) -> None:
        self._run = run
        self._state = state
        self._result = result
    ...
    def _reset_rect() -> QRect:
        return QRect(
            BUTTON_MARGIN + BUTTON_WIDTH + BUTTON_GAP, BUTTON_TOP, BUTTON_WIDTH, BUTTON_HEIGHT
        )
    ...
    def paintEvent(self, _event) -> None:
        ...
        state = {"armed": "Armed", "running": "Timing", "done": "Finished"}.get(self._state, "")
        if state:
        ...
        self._draw_button(painter, self._arm_rect(), "Disarm" if armed else "Arm", "arm", armed)
        self._draw_button(painter, self._reset_rect(), "Reset", "reset", False)

    def _draw_button(
        self, painter: QPainter, rect: QRect, label: str, key: str, active: bool
    ) -> None:
        path = QPainterPath()
        path.addRoundedRect(
            float(rect.x()), float(rect.y()), float(rect.width()), float(rect.height()), 7, 7
        )
        if active:
            fill = BUTTON_ARMED

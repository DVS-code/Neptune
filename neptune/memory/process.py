...
class Process:
    ...
    def attach(cls, exe_name: str) -> Process:
        ...
        # Raises when the process is not elevated, which is the normal case for a
        # first launch. Opening the game may still succeed, and if it does not the
        # checks below produce Neptune's own "run as administrator" message — far
        # more useful than a raw WinError 1314 escaping to the user.
        try:
    ...
    def close(self) -> None:
        # The handle is cleared even if the close itself fails. Leaving it set would
        # make `alive` keep reporting on a handle we have given up, and detach would
        # never complete.
        if self.handle:
            _close(self.handle)
            self.handle = 0
    ...
    def threads_suspended(self):
        ...
        try:
            ...
            # Every thread must be resumed even if one of them fails. ResumeThread
            # raises now that it goes through ntdll, and letting that propagate out of
            # the loop would leave the rest of the game's threads suspended — a hang.
...
def _module_base(pid: int, exe_name: str, handle: int = 0) -> int | None:
    ...
    if snapshot != -1:
        try:
            ...
            _close(snapshot)
    # we could not find the module with the toolhelp snapshot, so fall back to psapi

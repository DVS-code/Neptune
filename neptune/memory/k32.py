...
# ...
# EDIT: Don't use Microslop documentation...
...
STATUS_INFO_LENGTH_MISMATCH = -1073741820  # 0xC0000004 as signed LONG
...
# We might also need that later
# ntdll.NtAllocateVirtualMemory.restype = NTSTATUS
# ntdll.NtAllocateVirtualMemory.argtypes = [wintypes.HANDLE, ctypes.POINTER(ctypes.c_void_p), ctypes.c_size_t, ctypes.POINTER(ctypes.c_size_t), wintypes.ULONG, wintypes.ULONG]
...
def _query_process_modules(pid):
    ...
    try:
        ...
        for _ in range(4096):  # safety cap against a corrupted/racy list
...
def _snapshot_advance(entries, index_key, state, entry_out, fill):
    ...
    if entry_out.dwSize < ctypes.sizeof(type(entry_out)):
        raise ctypes.WinError(122)  # ERROR_INSUFFICIENT_BUFFER
